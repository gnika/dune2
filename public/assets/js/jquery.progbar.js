    ...
    $.fn.progBar = function(params) {
        ...
        this.each(function(){
			...
			var timer = setInterval(function(){
				updateProgressBar(value, t, params, timer);
			})// joachim
			// alert('e');	
            ...
            if(params.bubbles.active !== false) {
                ...
                // setInterval(function(){/*joachim retire bulles*/
...
function updateProgressBar(value, t, params, timer){
    ...
    if(params.texts != null && params.texts != "" && params.texts != {}){
		...
		if(indice != params.temoin){

			var request2 = $.ajax({
			url: '/majheure',
			data: { heure: value, cleHeure: indice },
			type: "POST",
			});
			request2.done(function(msg) {//quand on change d'heure
				var myvar = msg;
				var arr = myvar.split(':::');
				$("#nbJour").html(arr[0]);
				$("#nbSpice").html(arr[1]);
				...
				$("#nbCredit").html(arr[4]);
				$("#nbTroupe").html(arr[5]);
				$(".skills .serviteur").attr('title', arr[6]+' serviteurs');//serviteur
				$(".skills .influence").attr('title', arr[7]+' influences');//influence
				$(".skills .escorte").attr('title', arr[8]+' escortes');//gardien
				// $("span#nbSpice").html(function(i, h) {
					// return h.replace(/(.)/g, '<img src="images/chiffres/$1$1.png" />');
				// });
			});
			params.temoin=indice;
		}

        t.find(".progBarText").html(params.texts[indice]);
