...
var FullscreenCanvas = function () {
    ...
    FullscreenCanvas.prototype.renderIntoView = function renderIntoView() {
        ...
        var target = arguments.length <= 0 || arguments[0] === undefined ? document.getElementById('carte') : arguments[0];
        var canvas = this.canvas;
        ...
        canvas.style.top = '0px';/*border-radius: 118px;*/
...
function handlePointer() {
    ...
    function isSameCell(i, j) {
        ...
        return i === li && j === lj; // plus d'electrons bleus au passage
    };
    ...
    function print(isMove, _ref7) {
		...
		var divPos = {};/*AJOUT JOACHIM*/
		var offset = $("#carte").offset();
		if( offset !== undefined)
		divPos = {
			left: _ref7.pageX - offset.left,
			top: _ref7.pageY - offset.top
		};


        // var i = Math.floor(clientY / CELL_DISTANCE);
        // var j = Math.floor(clientX / CELL_DISTANCE;
        var i = Math.floor(divPos.top / CELL_DISTANCE);// joachim fait main
        var j = Math.floor(divPos.left / CELL_DISTANCE);
...
var shape = {
    ...
    init: function init() {
        ...
        var container = arguments.length <= 0 || arguments[0] === undefined ? document.getElementById('carte') : arguments[0];
...
document.getElementById('buttCarte').addEventListener('click', function () {
    var target = document.getElementById('inputValue');
	...
	if(value=="#galaxy"){
		$('.zero').hide();
		$('.loin').addClass('no_loin');
		$('.no_loin').removeClass('loin');
	}else{
		$('.zero').show();
		$('.no_loin').addClass('loin');
		$('.loin').removeClass('no_loin');
	}
	switch (value) {
		case '#destroy':
...
document.getElementById('inputValue').addEventListener('keypress', function (_ref15) {
