...
function animation(map, x, y, imageName, etat, texte){
...
function animationBuild(map, x, y, imageName, etat, texte, buildingPassed){
...
Game.init = function () {
	...
	objets['route_sable'] = {'img':'route_sable', 'name':'route_sable', 'life':100, 'possession':0, 'equipe':0};
	objets['route_dune']  = {'img':'route_dune', 'name':'route_dune', 'life':100, 'possession':0, 'equipe':0};
	objets['depierre'] 	  =	{'img':'depierre', 'name':'depierre', 'life':100, 'possession':0, 'equipe':0};
	objets['deracine'] 	  =	{'img':'deracine', 'name':'deracine', 'life':100, 'possession':0, 'equipe':0};

	batimentsHeros['hydraulique'] = 1;
	batimentsHeros['caserne'] = 0;
	batimentsHeros['ecole'] = 0;

    this.hero = new Hero(map, 160, 160, lifeHero, 60, 15, 5, 0, objets, batimentsHeros);//map - x - y - vie - attaque - defense - xp - objet
	...
	window.oncontextmenu = function (event){	//click droit
		var rect = document.getElementById("map_canvas").getBoundingClientRect();
		...
		if(rowClick == rowHero && colHero == colClick && batimentclick == 0 && dialogue == 0 && menuclick == 0	&& menussclickPlaceBatiment == 0){
			menuOutils = 1;
		}
	...
	document.getElementById("map_canvas").addEventListener('click',
		function(event){
			...
			var xClick = event.clientX - rect.left+Game.camera.x;
			var yClick = event.clientY - rect.top+Game.camera.y;
			var xClickCadre = event.clientX - rect.left;
			var yClickCadre = event.clientY - rect.top;
			var rowClick = Game.hero.map.getRow(xClick);
			var colClick = Game.hero.map.getCol(yClick);
			var rowHero = Game.hero.map.getRow(Game.hero.x);
			var colHero = Game.hero.map.getCol(Game.hero.y);
			...
			var pos     = map.getRow(yClick)*map.rows+map.getCol(xClick);
			var posHero = colHero * map.cols + rowHero;
			var tileClicked = map.getTile(1, rowClick, colClick);
			...
			if(abs2[posHero] == 191 && tileClicked == 191){//loupe si heros sur village
				returnVillages = 1;
			}

			if(rowHero == 10 && colHero == 7 && rowClick == 10 && colClick == 7){//si au pied de l'arbre on change de piece
				returnArbre = 1;
			}

			Object.keys(objets).forEach(function(key) {
				if(Game.hero.equipement[key].possession == 1 && Game.hero.equipement[key].life > 0){
					if(xClickCadre >  150 + n  && xClickCadre < 250 + n
						&&
						yClickCadre < 80 ){
			...
			if(menussclickPlaceBatiment == 1){ 		//placement de batiment
				...
				Object.keys(PlaceBatiment).forEach(function(key) {
					if(	pos == PlaceBatiment[key].pos && PlaceBatiment[key].img == 'buildOn' ){
						...
						payeRessources(PlaceBatiment['batiment'].supply);
						...
						Game.nameBuild = new Building(
							...
							rowClick,
							colClick,
							PlaceBatiment['batiment'].life,
							nameBuild,
							PlaceBatiment['batiment'].typeBatiment,
							PlaceBatiment['batiment'].caracteristique,
							PlaceBatiment['batiment'].typeTile,
							PlaceBatiment['batiment'].solidP,
							PlaceBatiment['batiment'].titre
						);
			...
			if(xClick < 60 && yClick > 170 && yClick < 170+64 && menuclick == 0 && menuBodyClick == 0){
				menuclick = 1;
			}
			else if(xClick < 60 && yClick >170 && yClick < 170+64 && menuclick == 1 && menuBodyClick == 0){
				menuclick = 0;
				menussclick=0;
			}
			...
			if(menussclick == 0 && batimentclick == 0 && menuBodyClick == 0 && dialogue == 0){	// bouge si aucun menu n'est ouvert
					path		 = [];
					if (xClick < 75 || yClick < 75)//quand on clique sur la bordure rouge
						return false;
			...
			if(batimentclick == 1){
				if( xClick > rect.width/4 && xClick < 200 + rect.width/4
				&&  yClick > rect.height/4 +100 &&  yClick <  rect.height/4 + 125){ //active desactive batiment
					if( builds[map.getRow(Game.hero.x)+'-'+map.getCol(Game.hero.y)].etat == 1 )
						builds[map.getRow(Game.hero.x)+'-'+map.getCol(Game.hero.y)].etat = 0;
					else
						builds[map.getRow(Game.hero.x)+'-'+map.getCol(Game.hero.y)].etat = 1;
				}
			...
			if(menuBodyClick == 1){
				...
				for(var i =0; i< Game.hero.artefact.length; i++){
					if( xClick > rect.width/4+ incr && xClick < 32 + rect.width/4 + incr
						&&  yClick > rect.height/4+20+180 + nbLigne &&  yClick <  rect.height/4 + 64+20+180 + nbLigne){ //ko
			...
			if(menussclick!=0){
				...
				if(xClick >map.tsize*7 && xClick <map.tsize*7+32 && yClick >map.tsize*2+10 &&  yClick < map.tsize*2+10+32 ){ //ok
					Game.hero.addBuild(
						...
						Game.hero.y,
						map,
						allBuilding[keySelected].paramBuild['typeBatiment'],
						allBuilding[keySelected].caracteristique,
						allBuilding[keySelected].supplyBuild,
						allBuilding[keySelected].paramBuild['typeTile'],
						allBuilding[keySelected].paramBuild['life'],
						allBuilding[keySelected].paramBuild['titre'],
						allBuilding[keySelected].paramBuild['solid']
					);
				}
...
Game._removeEquipe = function () {
	Object.keys(objets).forEach(function(key) {
		if(key == 'depierre' && Game.hero.equipement[key].equipe == 1)
			map.tileSolid.push(155);
	...
	if(layer == 1){
		...
		Object.keys(builds).forEach(function(key) {//barre de vie
			if(builds[key].caracteristique['showLife']==1)
				Game._drawRectangle('green', builds[key].row*map.tsize-Math.round(Game.camera.x), builds[key].col*map.tsize-Math.round(Game.camera.y)+map.tsize+5, builds[key].life);
		})
		//sprite personnages
...
Game._drawGridMenu = function () {
	...
	Object.keys(builds).forEach(function(key) {// pour les bulles decompte de batiment / routes de liaison
		...
		if(builds[key].batiment.length >1){
			...
			if(builds[key].road == '')
				position = 'no_road';
			//mettre image arret production si on peut arrêter la prod
			...
			if( builds[key].etat == 0)
			Game.ctx.drawImage(
				Loader.getImage('stop'), // image
				0, // source x
				0, // source y
				map.tsize, // source width
				map.tsize, // source height
	...
	//emplacement batiment constructible
	...
	if(typeof(Game.animArray) != "undefined" &&  Game.animArray.length > 0) {
		var animArray = Game.animArray;

		for (var i = 0; i < Game.animArray.length; i++) {
			var animation = 'Game.animBref'+i;

			if (animArray[i].buildAnim.animationBuild <= DUREE_ANIMATION) {
				if (animArray[i].etat !== null && animArray[i].etat != '')
					Game.ctx.drawImage(
						animArray[i].etat, // image
						0, // source x
						0, // source y
						map.tsize, // source width
						map.tsize, // source height
						animArray[i].x - 30 - Game.camera.x,  // target x
						animArray[i].y - 15 - Game.camera.y - animArray[i].buildAnim.animationBuild,
						map.tsize, // target width
						map.tsize // target height
					);

				if (typeof (animArray[i].images) != "undefined") {

					var n = 0;
					for (var i = 0; i < animArray[i].images.length; i++) {
						Game.ctx.drawImage(
							Loader.getImage(animArray[i].images[i]), // image
							0, // source x
							0, // source y
							map.tsize, // source width
							map.tsize, // source height
							animArray[i].x - 15 - Game.camera.x - n,  // target x
							animArray[i].y - 15 - Game.camera.y - animArray[i].buildAnim.animationBuild,
							map.tsize, // target width
							map.tsize // target height
						);
					...
					Game.ctx.drawImage(
						animArray[i].image, // image
						0, // source x
						0, // source y
						map.tsize, // source width
						map.tsize, // source height
						animArray[i].x - 15 - Game.camera.x,  // target x
						animArray[i].y - 15 - Game.camera.y - animArray[i].buildAnim.animationBuild,
						map.tsize, // target width
						map.tsize // target height
					);
				...
				if (typeof (animArray[i].texte) != 'undefined') {
					Game.ctx.font = "20px Arial";
					Game.ctx.fillStyle = 'black';
					Game.ctx.fillText(animArray[i].texte, animArray[i].x - 30 - Game.camera.x, animArray[i].y - Game.camera.y - animArray[i].buildAnim.animationBuild);
				}

				animArray[i].buildAnim.animationBuild ++;
				animArray[i].buildAnim.animationBuild ++;

			} else {
				animArray[i].buildAnim.animationBuild = 0;
				Game.animArray.splice(i, 1);
			}
		}
	}
	...
	if(typeof(anim) != "undefined" && anim !== null) {
		if(Game.animBref<=DUREE_ANIMATION){
				...
				if(typeof(anim.texte) !='undefined'){
					Game.ctx.font="20px Arial";
					Game.ctx.fillStyle = 'black';
					Game.ctx.fillText(anim.texte, anim.x-30-Game.camera.x, anim.y-Game.camera.y-Game.animBref);
				}
			Game.animBref++;
			Game.animBref++;
		}else{
...
Game.render = function () {
	...
	Object.keys(artefacts).forEach(function(key) {
		if(absobs1[artefacts[key].col * map.rows + artefacts[key].row] != 7){//EXPLORATION
			...
			if( Loader.getImage('mini_'+artefacts[key].image) )
				var imageArt = Loader.getImage('mini_'+artefacts[key].image)
			else
				var imageArt = Loader.getImage(artefacts[key].image)
	...
	pos = map.getRow(this.hero.y)*map.rows+map.getCol(this.hero.x);
	if(abs2[pos] == 191){//loupe si heros sur village
		Game.ctx.drawImage(
