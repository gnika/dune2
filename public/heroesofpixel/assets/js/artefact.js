...
function Artefact(map, x, y, row, col, attaque, defense, agilite, xp, exploration, supply, duree, inventaire, image1, quete, outils, text_prendre) {
	this.map = map;
    this.xp = xp;
    ...
    this.quete = 1;
    this.outils = outils;
    this.text_prendre = text_prendre;
}
...
function generateArtefact(map, x, y, row, col, name){
	...
	this.nameArtefact = new Artefact(map, x, y, row, col, allArtefacts[name].attaque, 
														...
														allArtefacts[name].text_prendre);
...
Hero.prototype.bombe = function () {	// utiliser la bombe
	...
	if( abs2[colY*map.rows+rowX+1] != 156 )
		abs2[colY*map.rows+rowX+1] = 184;	//tuile lunaire
	if( abs2[colY*map.rows+rowX-1] != 156 )
		abs2[colY*map.rows+rowX-1] = 184;	//tuile lunaire
	if( abs2[colY*map.rows+rowX+map.rows] != 156 )
		abs2[colY*map.rows+rowX+map.rows] = 184;	//tuile lunaire
	if( abs2[colY*map.rows+rowX-map.rows] != 156 )
		abs1[colY*map.rows+rowX-map.rows] = 184;	//tuile lunaire
