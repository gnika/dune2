...
function paysanMove(delta){
	Object.keys(builds).forEach(function(key) {
		if(builds[key].road.length >0 && builds[key].paysan_fois > 0 && builds[key].etat == 1){
			...
			if( builds[key].level == 1)
				var coef = 1;
			else if( builds[key].level == 2)
				coef = 1.5;
			else
				coef = 2;
			...
			if(builds[key].dirxPaysan == 1){//vers la droite
				if(builds[key].paysan_position.x > builds[key].positionXPaysan)
					...
					builds[key].paysan_position.x += builds[key].dirxPaysan * builds[key].paysan_vitesse * delta * coef;
			}
			...
			if(builds[key].diryPaysan == 1){//descend
				if(builds[key].paysan_position.y > builds[key].positionYPaysan)
					...
					builds[key].paysan_position.y += builds[key].diryPaysan * builds[key].paysan_vitesse * delta * coef;
			}
			...
			if( builds[key].roady.length == 0){
				...
				if(builds[key].paysan_retour == 0){//atteint son but			
					...
					if((typeof builds[key].caracteristique['prixUpdate'] != "undefined") ) {
						var updatePrice = builds[key].caracteristique['prixUpdate'];
						var recompense = builds[key].caracteristique['recompense'];
					}
					else
						updatePrice = 0;
					...
					if(updatePrice != 0){	//si on doit payer des ressources pour passer d'un niveau à l'autre - exemple : moulin doit payer 100 blé pour faire de la farine
						...
						if(error == 0){
							...
							payeRessources(updatePrice);
							payeRessources(recompense, 1);

							animRessources = new animationBuild(map, builds[key].x+map.tsize, builds[key].y, Object.keys(builds[key].caracteristique['recompense'])[0], 'plus', '', builds[key]);
							Game.animArray.push(animRessources);
